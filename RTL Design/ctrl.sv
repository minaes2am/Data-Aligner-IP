`ifndef CTRL_SV
  ...
  module ctrl #(
    ...
    input                        ctrl_wr_strobe    // 1-cycle pulse when CTRL register is written; resets accumulators
    );
    ...
    always@(posedge clk or negedge reset_n) begin
      if(reset_n == 0) begin
        ...
        // ------------------------------------------------------------------
        // When the CTRL register is written with a new (size, offset) pair,
        // regs.sv pulses ctrl_wr_strobe for one clock cycle.  Reset the
        // partial-accumulation state so that the new ctrl_size/ctrl_offset
        // values are applied from a clean baseline.  Any bytes that were
        // partially accumulated under the old configuration are discarded,
        // mirroring the model's flush_partial_buf() call in the test.
        // ------------------------------------------------------------------
        if(ctrl_wr_strobe) begin
          push_valid                <= 0;
          push_data                 <= 0;
          unaligned_offset          <= 0;
          unaligned_size            <= 0;
        ...
        else if((push_valid == 1) & (push_ready == 0)) begin
