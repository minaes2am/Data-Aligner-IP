`ifndef ALGN_REG_PKG_SV
  ...
  class algn_reg_ctrl extends uvm_reg;
    ...
    constraint legal_size {
      SIZE.value != 0;
      // SIZE field is 3 bits: valid values 1..4 for a 32-bit design (ALGN_DATA_WIDTH/8 = 4 bytes)
      // Ensure SIZE does not exceed the number of bytes in the data width
      SIZE.value <= (ALGN_DATA_WIDTH / 8);
    }
    
    constraint legal_size_offset {
      // Match DUT rx_ctrl.sv check: ((ALGN_DATA_WIDTH/8) + offset) % size == 0
      ((ALGN_DATA_WIDTH / 8) + OFFSET.value) % SIZE.value == 0;
      // Match DUT regs.sv check: offset + size <= data_width_bytes
