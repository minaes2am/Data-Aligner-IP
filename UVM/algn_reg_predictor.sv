`ifndef ALGN_REG_PREDICTOR_SV
  `define ALGN_REG_PREDICTOR_SV
  ...
  class algn_reg_predictor#(type BUSTYPE = uvm_sequence_item) extends uvm_reg_predictor#(.BUSTYPE(BUSTYPE));
    
    //Pointer to the environment configuration
    algn_env_config env_config;

    `uvm_component_param_utils(algn_reg_predictor#(BUSTYPE))
    ...
    protected virtual function algn_reg_access_status_info get_exp_response(uvm_reg_bus_op operation);
      uvm_reg register;
      ...
      if(register == null) begin
        return algn_reg_access_status_info::new_instance(UVM_NOT_OK, "Access to a location on which no register is mapped");
      end
      ...
      if(operation.kind == UVM_WRITE) begin
        ...
        if(info.rights == "RO") begin
          return algn_reg_access_status_info::new_instance(UVM_NOT_OK, "Write access to a full read-only register");
        end
      end
      ...
      if(operation.kind == UVM_READ) begin
        ...
        if(info.rights == "WO") begin
          return algn_reg_access_status_info::new_instance(UVM_NOT_OK, "Read access from a full write-only register");
        end
      end
      ...
      if(operation.kind == UVM_WRITE) begin
        ...
        if($cast(ctrl, register)) begin
          ...
          if(size_value == 0) begin
            return algn_reg_access_status_info::new_instance(UVM_NOT_OK, "Write value 0 to CTRL.SIZE");
          end
          
          //Trying to write SIZE > data_width_bytes will return an APB error.
          if(size_value > (env_config.get_algn_data_width() / 8)) begin
            return algn_reg_access_status_info::new_instance(UVM_NOT_OK,
              $sformatf("Write SIZE=%0d exceeds max allowed %0d for data_width=%0d",
                        size_value, env_config.get_algn_data_width()/8, env_config.get_algn_data_width()));
          end
          ...
          if(((env_config.get_algn_data_width() / 8) + offset_value) % size_value != 0) begin
            return algn_reg_access_status_info::new_instance(UVM_NOT_OK, $sformatf("Illegal access to CTRL -> OFFSET: %0d, SIZE: %0d, aligner data width: %0d",
                                                                offset_value, size_value, env_config.get_algn_data_width()));
          end
          
          //Trying to write OFFSET + SIZE > data_width_bytes will return an APB error.
          if(offset_value + size_value > (env_config.get_algn_data_width() / 8)) begin
            return algn_reg_access_status_info::new_instance(UVM_NOT_OK,
              $sformatf("Write OFFSET=%0d + SIZE=%0d exceeds data_width_bytes=%0d",
      ...
      return algn_reg_access_status_info::new_instance(UVM_IS_OK, "All OK");
    endfunction
    ...
    virtual function void write(BUSTYPE tr);
      ...
      if(env_config.get_has_checks()) begin
        algn_reg_access_status_info exp_response = get_exp_response(operation);
