...
module testbench();
  ...
  initial begin
   ...
   uvm_config_db#(virtual apb_if)::set(null, "uvm_test_top.env.apb_agt", "vif", apb_if);
    ...
    uvm_config_db#(virtual md_if#(ALGN_DATA_WIDTH))::set(null, "uvm_test_top.env.md_rx_agent", "vif", md_rx_if);
    uvm_config_db#(virtual md_if#(ALGN_DATA_WIDTH))::set(null, "uvm_test_top.env.md_tx_agent", "vif", md_tx_if);
    
    uvm_config_db#(virtual algn_if)::set(null, "uvm_test_top.env", "vif", algn_if);
    
    //Start UVM test and phases
    run_test("algn_test_random");
  end
  
  //Instantiate the DUT
  aligner#(.ALGN_DATA_WIDTH(`ALGN_TEST_ALGN_DATA_WIDTH)) dut(
    .clk(    clk),
