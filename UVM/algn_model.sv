`ifndef ALGN_MODEL_SV
  ...
  class algn_model extends uvm_component implements uvm_ext_reset_handler;
    ...
    protected virtual function md_response get_exp_response(md_item_mon item);
      ...
      // NOTE: This is the ONLY error check the DUT (rx_ctrl.sv) performs beyond size==0.
      // The DUT does NOT check offset+size > data_width/8, so neither should the model.
        ...
        begin
          fork
            ...
            begin
              repeat(10) begin
                 @(posedge vif.clk);
                 if(!(reg_block.STATUS.RX_LVL.get_mirrored_value() < rx_fifo.size())) continue;
                break;
              end
        ...
        begin
          fork
            ...
            begin
              repeat(10) begin
                 @(posedge vif.clk); 
                 if((reg_block.STATUS.RX_LVL.get_mirrored_value() > 0) && (reg_block.STATUS.TX_LVL.get_mirrored_value() < tx_fifo.size()))continue;
                break; 
              end
        ...
        begin
          fork
            ...
            begin
              repeat(10) begin
                 @(posedge vif.clk);
                 if(reg_block.STATUS.TX_LVL.get_mirrored_value() < tx_fifo.size())continue;
                break;   
              end
        ...
        begin
          fork
            ...
            begin
              repeat(200) begin
                 @(posedge vif.clk); 
                 if(reg_block.STATUS.TX_LVL.get_mirrored_value() > 0)continue;
                break;   
              end
    ...
    protected virtual task pop_from_rx_fifo(ref md_item_mon item);
    ...
    protected virtual task pop_from_tx_fifo(ref md_item_mon item);
    ...
    protected virtual function void split(int unsigned num_bytes, md_item_mon item, ref md_item_mon items[$]);
      ...
      for(int i = 0; i < 2; i++) begin
        md_item_mon splitted_item = md_item_mon::type_id::create("splitted_item", this);
    ...
    virtual function void write_in_rx(md_item_mon item_mon);
      if(item_mon.get_end_time() == 0) begin
