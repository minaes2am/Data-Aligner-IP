`ifndef APB_PKG_SV
  ...
  import uvm_ext_pkg::*;
    ...
    class apb_coverage extends uvm_ext_coverage#(.VIRTUAL_INTF(apb_vif), .ITEM_MON(apb_item_mon));
      ...
      covergroup cover_item with function sample(apb_item_mon item);
        ...
        length : coverpoint item.length {
          option.comment = "Length of the APB access. DUT regs.sv always deasserts pready on SETUP cycle so min length=3. length=2 (no wait states) is structurally unreachable.";
          bins length_le_4     = {[3:4]};

          illegal_bins length_lt_2        = {[$:1]};
          ignore_bins  length_unreachable = {2, [5:$]};
        }
        ...
        response_x_direction : cross response, direction {
          // The DUT never returns APB_ERR on a READ access in the current
          // register map; mark the combination as ignored to avoid a 0% bin.
          ignore_bins ignore_err_read = binsof(response) intersect {APB_ERR} &&
                                        binsof(direction) intersect {APB_READ};
        }
        ...
        trans_direction : coverpoint item.dir {
          ...
          // APB_READ=>APB_READ is structurally hard to hit because the test sequences
          // always interleave writes and reads; mark as ignored to avoid a 0% bin.
          ignore_bins ignore_read2read = (APB_READ => APB_READ);
        }
    ...
    class apb_driver extends uvm_ext_driver#(.VIRTUAL_INTF(apb_vif), .ITEM_DRV(apb_item_drv));
    ...
    class apb_agent extends uvm_ext_agent#(.VIRTUAL_INTF(apb_vif), .ITEM_MON(apb_item_mon), .ITEM_DRV(apb_item_drv));
