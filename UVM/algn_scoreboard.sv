`ifndef ALGN_SCOREBOARD_SV
  ...
  class algn_scoreboard extends uvm_component implements uvm_ext_reset_handler;
    ...
    //Counter tracking how many times each RX item (by begin_time) has been received
    protected int unsigned rx_item_write_count[int];  // keyed by item inst_id
    
    //Counter tracking how many times each TX item (by begin_time) has been received
    protected int unsigned tx_item_write_count[int];  // keyed by item inst_id
    ...
    virtual function void handle_reset(uvm_phase phase);
      ...
      exp_tx_items.delete();
      exp_irqs.delete();
      rx_item_write_count.delete();
      tx_item_write_count.delete();
    ...
    virtual function void write_in_agent_rx(md_item_mon item_mon);
      int key = item_mon.get_inst_id();
      rx_item_write_count[key]++;
      if(rx_item_write_count[key] < 2) begin
        return; // first write = item active, ignore
      end
      rx_item_write_count.delete(key); // second write = item complete, process it
      
      if(exp_rx_responses.size() == 0) begin
        if(env_config.get_has_checks()) begin
          `uvm_error("DUT_ERROR", $sformatf("Received RX response with no expected entry in queue - item: %0s", item_mon.convert2string()))
        end
        return;
      end
    ...
    virtual function void write_in_agent_tx(md_item_mon item_mon);
      int key = item_mon.get_inst_id();
      tx_item_write_count[key]++;
      if(tx_item_write_count[key] < 2) begin
        return; // first write = item active, ignore
      end
      tx_item_write_count.delete(key); // second write = item complete, process it
